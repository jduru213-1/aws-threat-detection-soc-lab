...
class ThreadLocalLoggingStack:
    ...
    It provides interfaces like a usual stack data structure. Since it
    leverages thread local, there is one and only one instance of this
    stack can be created per thread
    """
    ...
    @classmethod
    def empty(cls):
        return not (hasattr(cls._data, "logging_context_stack") and cls._data.logging_context_stack)
...
class ContextLoggerAdapter(LoggerAdapter):
    ...
    This class fetches the latest context from thread local context
    stack and supports logging the context in a k-v mode.
    """
...
def setup_root_logger(app_name=None, modular_name=None, stanza_name=None, logging_level=WARNING):
    """Helper function for root logger setup, called only once for each
    process.
...
class RotatingFileHandlerFactory:
    ...
    def __call__(self):
        ...
        handler = RotatingFileHandler(filepath, maxBytes=1024 * 1024 * 25, backupCount=5, delay=True)
        return handler
