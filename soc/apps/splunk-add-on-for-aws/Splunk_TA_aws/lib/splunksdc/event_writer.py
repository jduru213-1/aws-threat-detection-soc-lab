...
class XMLEventWriter(EventWriter):
    ...
    _CHUNK_TEMPLATE = (
        '<stream><event unbroken="1" {stanza}>{source}{sourcetype}{host}{index}{data}{done}</event></stream>'
    )
    ...
    def _render_defaults(cls, kwargs):
        defaults = dict.fromkeys(cls._METADATA_KEYS, "")
        defaults["stanza"] = ""
...
class HECWriter(EventWriter):
    ...
    def __init__(self, url, **kwargs):
        ...
        self._auth_header = {"Authorization": f"Splunk {token}"}
