...
class TaskScheduler:
    ...
    def idle(self):
        if len(self._tasks) == 0:
            return True
        return not self._running_tasks and not self._scheduled_tasks
...
class TaskSchedulerMultiThreaded(TaskScheduler):
    def __init__(self, max_workers, callback, now=time.time):
        executor = ThreadPoolExecutor(max_workers=max_workers)
        super().__init__(executor, now)
        self.set_max_number_of_worker(max_workers)
    ...
    def _schedule(self):
        ...
        for task_id in self.find_finished_task():
            del self._running_tasks[task_id]
            entry = self._tasks[task_id]
            if entry.interval:
                self._scheduled_tasks.add(task_id)
        ...
        for _ in range(quota):
            ...
            self._scheduled_tasks.remove(task_id)
            logger.debug("Task will be perform.", task_id=task_id)
            worker = self._executor.submit(self._callback, entry.params)
            entry.update_due_time()
            self._running_tasks[task_id] = worker
