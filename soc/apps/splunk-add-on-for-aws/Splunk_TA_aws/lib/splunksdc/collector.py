...
from splunksdc.file_lock import file_lock
...
def _validate_definition(stream, scheme):
    return 0 if ValidationDefinition.parse(stream) is not None else 0
...
def run_modular_input(modular_input_factory, **kwargs):
    ...
    context = Context.from_url(args.server_uri, args.stanza) if args.server_uri else Context.from_stream(sys.stdin)
...
class SimpleCollectorTask:
    ...
    def _task_procedure(cls, identifier, master_context, callback, app, name, params):
        prefix = "" if os.name != "nt" else master_context
...
class SimpleCollectorV1:
    ...
    def setup_root_logger(self, shard=None):
        ...
        factory = logging.RotatingFileHandlerFactory(self._app_name, self._modular_name, stanza_name)
        logging.RootHandler.setup(factory)

    def _lock_file_path(self, path) -> str:
        filename = ""
        if not self._use_single_instance:
            stanza = self.inputs()[0]
    ...
    def run(self) -> int:
        ...
        run_lock_file = self._lock_file_path(folder)
        with file_lock(run_lock_file) as lock_result:
